...
public class teleporterRedstonePlate extends Teleporter {
			...
			for(int l = -2; l <= 2; ++l){
				for(int i1 = -2; i1 <= 2; ++i1){
					for(int j1 = -1; j < 3; ++j1){
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlock(k1, l1, i2, flag ? blockRegistry.BlockRspDWall : Blocks.air);
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8){
		...
		if(this.destinationCoordinateCache.containsItem(j1)){
			teleporterRedstonePlate.PortalPosition portalposition = (Teleporter.PortalPosition)this.destinationCoordinateCache.getValueByKey(j1);
			d3 = 0.0D;
			...
			for(l3 = l - short1; l3 <= 1 + 128; ++l3){
				...
				for(int l1 = i1 - short1; l1 <= i1 + short1; ++l1){
					...
					for(int i2 = this.worldServerInstance.getActualHeight() -1; i2 >=0; --i2){
						if(this.worldServerInstance.getBlock(l3, i2, l1) == blockRegistry.BlockRspDWall){
							while (this.worldServerInstance.getBlock(l3, i2 - 1, l1) == blockRegistry.BlockRspDWall){
								--i2;
							}
		...
		if (d3 >= 0.0D){
			if(flag){
				this.destinationCoordinateCache.add(j1, new teleporterRedstonePlate.PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			if(this.worldServerInstance.getBlock(i - 1, j, k) == blockRegistry.BlockRspDWall){
				i4 = 2;
			}
			...
			if(this.worldServerInstance.getBlock(i, j, k - 1) == blockRegistry.BlockRspDWall){
				i4 = 3;
			}
